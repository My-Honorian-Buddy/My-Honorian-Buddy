...
<div class="bw-progress-bar {{$class}}">
    ...
    <div @class([
        ...
        <div style="width: {{$percentage}}%"
             class="text-center py-1 {{$bar_colour}} {{$cssOverride}} relative border border-charcoal overflow-hidden h-full rounded-full bar-width animate__animated animate__fadeIn {{$barClass}}">
            @if($showPercentageLabel && $showPercentageLabelInline)
                <span class="text-{{$colour}}-{{$text_colour_weight[$shade]}} h-full dark:text-dark-600 px-2 text-xs inline-flex align-middle">
