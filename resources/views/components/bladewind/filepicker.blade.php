...
<div class="relative px-2 py-3 border-2 border-dotted border-black transition ease-in-out hover:border-solid dark:text-dark-300 dark:border-dark-600
    dark:bg-transparent hover:bg-accent2 hover:dark:border-dark-500 text-center cursor-pointer rounded-md bw-fp-{{ $name }} @if($add_clearing) mb-3 @endif">
    <x-bladewind::icon name="document-text"
                       class="h-6 w-6 absolute z-20 left-4 rtl:!right-4 rtl:!left-[unset] text-primary dark:text-dark-500"/>
    <x-bladewind::icon name="x-circle"
                       ...
                       type="solid"/>
    <span class="text-primary px-6 pl-10 zoom-out inline-block selection">
        {{ $placeholder }}
        @if($required)
...
<script>
    domEl('.bw-fp-{{ $name }}').addEventListener('drop', function (evt) {
        changeCss('.bw-fp-{{ $name }}', 'border-black', 'remove');
        changeCss('.bw-fp-{{ $name }}', 'border-black');
        evt.preventDefault();
        domEl('.bw-{{ $name }}').click();
    });
